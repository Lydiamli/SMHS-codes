...
def create_mean_columns(data):
    ...
    combine_mean_columns(data, 'Block_TD2', 'Block_TD3', 'Block_TD2TD3') #stratigies and interaction
...
def group_outcomes(data):
    ...
    outcome_data = outcome_data.drop(columns= ["ADHD_outcome","ODD_outcome","CD_outcome",
                    "Depression_outcome","Anxiety_outcome"])                      
...
def lgbm_gain_feature_importance(education, X_train, model_name, target_outcome, count): 
    save_path = f'../results/edu_feature_imp/{education}'
    ...
    gain_importance.to_csv(f"{save_path}/lgbm_{education}_{target_outcome}{count}.csv", index= False)

# permutation lgbm 
def permutation_feature_importance(education, X_train, X_test, y_test, model_name, target_outcome, count):
    save_path = f'../results/edu_feature_imp/{education}'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    perm_importance_df_sorted.to_csv(f"{save_path}/permutation_{education}_{target_outcome}{count}.csv", index= False)

#lgbm gain shapvic
def shap_feature_importance(education, X_train, X_test, pipeline, target_outcome, model_type, count):
    save_path = f'../results/edu_feature_imp/{education}'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    importance_df.to_csv(f"{save_path}/shap_{education}_{target_outcome}{count}.csv", index= False)

#%%
FILE = Path("../data/clean/untouched_dataset.csv")
d0 = pd.read_csv(FILE)
d0 = d0.drop(columns= "SF5ParentEd")
...
#%% load data
def ml_featureimportance(education, train_IDs, test_IDs):
    for count in range(10):
        FILE = Path(f"../data/clean/imputed_train/imputed_train{count}.csv")
        train_df = pd.read_csv(FILE)
        train_df['SF5ParentEd'] = np.where(train_df['SF5ParentEd'] <= 2, 0, 1)

        FILE = Path(f"../data/clean/imputed_test/imputed_test{count}.csv")
        test_df = pd.read_csv(FILE) 
        test_df['SF5ParentEd'] = np.where(test_df['SF5ParentEd'] <= 2, 0, 1)

        if education == "lowEd":
            train_df = train_df[train_df['SF5ParentEd'] == 0]
            train_df = train_df.drop(columns= ['SF5ParentEd'])
            test_df = test_df[test_df['SF5ParentEd'] == 0 ]
            test_df = test_df.drop(columns= ['SF5ParentEd'])
        elif education == "highEd":
            train_df = train_df = train_df[train_df['SF5ParentEd'] == 1]
            train_df = train_df.drop(columns= ['SF5ParentEd'])
            test_df = test_df[test_df['SF5ParentEd'] == 1]
            test_df = test_df.drop(columns= ['SF5ParentEd'])
        ...
        train_df = train_df.drop(columns=['SA3Grade', 'PSCH_level'])
        test_df = test_df.drop(columns=['SA3Grade', 'PSCH_level'])

        # LOOP
        filename = f'../results/edu_model_results/{education}/{education}_model_results{count}.pkl'

        if not os.path.exists(f'../results/edu_model_results/{education}'):
            os.mkdir(f'../results/edu_model_results/{education}')
        ...
        for outcome_name in outcome_list:
            ...
            if best_model_type == "lgbm":
                lgbm_gain_feature_importance(education, X_train, lgbm_model, outcome_name, count)
            
            permutation_feature_importance(education, X_train, X_test, y_test,results_dict[outcome_name][best_model_type]['pipeline'], outcome_name, count)

            shap_feature_importance(education,X_train, X_test, results_dict[outcome_name][best_model_type]['pipeline'], outcome_name, best_model_type, count)



#%%
education_group = ["highEd", "lowEd"]
for education in education_group:
        ml_featureimportance(education, train_IDs, test_IDs)
#%%