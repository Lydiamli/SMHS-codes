...
#%%
def model_fit_results(mental_health, model, metrics_df): 
    ...
    # Loop over the 10*100 files
    for x in range(10):
        for y in range(100):
            if x ==9 and y >90:
                continue
            file_path = f"../results/model_results/model_results_{x}_{y}.pkl"
            try:
        ...
        std_train_mae = round(statistics.stdev(train_mae_values), 4) if len(train_mae_values) > 1 else None

        mean_train_mse = round(sum(train_mse_values) / len(train_mse_values), 4) if train_mse_values else None
        std_train_mse = round(statistics.stdev(train_mse_values), 4) if len(train_mse_values) > 1 else None


        mean_cv_mae = round(sum(cv_mae_values) / len(cv_mae_values), 4) if cv_mae_values else None
        std_cv_mae = round(statistics.stdev(cv_mae_values), 4) if len(cv_mae_values) > 1 else None

        mean_cv_mse = round(sum(cv_mse_values) / len(cv_mse_values), 4) if cv_mse_values else None
        std_cv_mse = round(statistics.stdev(cv_mse_values), 4) if len(cv_mse_values) > 1 else None


        mean_test_mae = round(sum(test_mae_values) / len(test_mae_values), 4) if test_mae_values else None
        std_test_mae = round(statistics.stdev(test_mae_values), 4) if len(test_mae_values) > 1 else None

        mean_test_mse = round(sum(test_mse_values) / len(test_mse_values), 4) if test_mse_values else None
        std_test_mse = round(statistics.stdev(test_mse_values), 4) if len(test_mse_values) > 1 else None
        ...
        metrics = {
            ...
            'mean_train_mae': mean_train_mae, 'std_train_mae': std_train_mae,
            'mean_train_mse': mean_train_mse, 'std_train_mse': std_train_mse,

            'mean_cv_mae': mean_cv_mae, 'std_cv_mae': std_cv_mae,
            'mean_cv_mse': mean_cv_mse, 'std_cv_mse': std_cv_mse,

            'mean_test_mae': mean_test_mae, 'std_test_mae': std_test_mae,
            'mean_test_mse': mean_test_mse, 'std_test_mse': std_test_mse}
...
for outcome in mental_health_outcomes:
    for model in models:
        metrics_df = model_fit_results(outcome, model, metrics_df)
