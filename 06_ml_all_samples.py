...
def group_outcomes(data):
    ...
    outcome_list = list(data.filter(regex = "_outcome$").columns)
...
# define functions for model training and evalutation
...
def evaluate_ridge(X_train, y_train, ID_train, X_test, y_test, ID_test):
    ...
    best_ridge_model = ridge_model.best_estimator_ #give the complete pipeline
...
def lgbm_gain_feature_importance(X_train, model_name, target_outcome, count): 
    save_path = f'../results/feature_imp'
    ...
    gain_importance.to_csv(f"{save_path}/lgbm_{target_outcome}{count}.csv", index= False)

# permutation lgbm 
def permutation_feature_importance(X_train, X_test, y_test, model_name, target_outcome, count):
    save_path = f'../results/feature_imp'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    perm_importance_df_sorted.to_csv(f"{save_path}/permutation_{target_outcome}{count}.csv", index= False)

#lgbm gain shapvic
def shap_feature_importance(X_train, X_test, pipeline, target_outcome, model_type, count):
    save_path = f'../results/feature_imp'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    importance_df.to_csv(f"{save_path}/shap_{target_outcome}{count}.csv", index= False)
#%%
...
#%% load data
def ml_featureimportance(train_IDs, test_IDs):
    for x in range(10):
        for y in range(100):
            if x ==9 and y >90:

                FILE = Path("../data/clean/imputed_train/mean_convergence_impution_correlations.csv")
                d0 = pd.read_csv(FILE) 
                mask = d0["mean_cor"].values < 0.5
                variables_with_r_less_than_50 = d0[mask]["variable"].tolist()

                FILE = Path(f"../data/clean/imputed_train/imputed_train{x}.csv")
                train_df = pd.read_csv(FILE) 
                train_df.drop(columns= variables_with_r_less_than_50, inplace= True)
                train_df.index = train_IDs.index.values
                    
                FILE = Path(f"../data/clean/imputed_test/imputed_test{y}.csv")
                test_df = pd.read_csv(FILE) 
                test_df = test_df.drop(columns= variables_with_r_less_than_50, axis=1)
                test_df.index = test_IDs.index.values
                ...
                columns_to_drop = train_df.filter(regex='.*(outcome|cluster)$').columns
                train_predictors = train_df.drop(columns=columns_to_drop)
                predictor_correlation = train_predictors.corr()

                # identify predictor pairs r > 0.7
                mask = np.tril(np.ones_like(predictor_correlation, dtype=bool))
                filtered_corr  = predictor_correlation.mask(mask)

                high_corr_pairs = []
                for col in filtered_corr.columns:
                    for row in filtered_corr.index:
                        if abs(filtered_corr.loc[row, col]) > 0.7:
                            high_corr_pairs.append((row, col))

                print(f"Highly correlated predictors: {high_corr_pairs}")
                #('SA2Age', 'SA3Grade'), ('SF3MomCan', 'SF4DadCan'), 
                #('SA2Age', 'PSCH_level'), ('SA3Grade', 'PSCH_level')
                
                # final features 
                demographics_cols.remove('SA3Grade')
                demographics_cols.remove('PSCH_level')
                feature_labels = demographics_cols + list(block_train.columns)
                print(len(feature_labels)) #87

                train_df = train_df.drop(columns=['SA3Grade', 'PSCH_level'], axis= 1)
                test_df = test_df.drop(columns=['SA3Grade', 'PSCH_level'], axis= 1)

                combined_train_test = pd.concat([train_df, test_df], axis=0)
                save_directory = '../data/clean/imputed_processed'
                if not os.path.exists(save_directory):
                    os.mkdir(save_directory)
                csv_filename = f"complete_{x}_{y}.csv" 
                csv_filepath = os.path.join(save_directory, csv_filename)
                combined_train_test.to_csv(csv_filepath, index=False)



                # LOOP
                filename = f'../results/model_results/model_results_{x}_{y}.pkl'

                if not os.path.exists(f'../results/model_results/'):
                    os.mkdir(f'../results/model_results/')
                ...
                for outcome_name in outcome_list:
                    ...
                    if best_model_type == "lgbm":
                        lgbm_gain_feature_importance(X_train, lgbm_model, outcome_name, x, y)
                    
                    permutation_feature_importance(X_train, X_test, y_test, results_dict[outcome_name][best_model_type]['pipeline'], outcome_name, x, y)

                    shap_feature_importance(X_train, X_test, results_dict[outcome_name][best_model_type]['pipeline'],  outcome_name, best_model_type, x, y)

# %%
ml_featureimportance(train_IDs, test_IDs)
