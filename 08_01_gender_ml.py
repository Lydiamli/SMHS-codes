...
def create_mean_columns(data):
    ...
    combine_mean_columns(data, 'Block_TD2', 'Block_TD3', 'Block_TD2TD3')
...
def get_train_test_data(train_df, test_df, feature_labels, target_outcome):
    train_df = train_df.dropna(subset=[target_outcome])
    test_df = test_df.dropna(subset=[target_outcome])
...
def lgbm_gain_feature_importance(gender, X_train, model_name, target_outcome, count): 
    save_path = f'../results/gender_feature_imp/{gender}'
    ...
    gain_importance.to_csv(f"{save_path}/lgbm_{gender}_{target_outcome}{count}.csv", index= False)

# permutation lgbm 
def permutation_feature_importance(gender, X_train, X_test, y_test, model_name, target_outcome, count):
    save_path = f'../results/gender_feature_imp/{gender}'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    perm_importance_df_sorted.to_csv(f"{save_path}/permutation_{gender}_{target_outcome}{count}.csv", index= False)

#lgbm gain shapvic
def shap_feature_importance(gender, X_train, X_test, pipeline, target_outcome, model_type, count):
    save_path = f'../results/gender_feature_imp/{gender}'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    importance_df.to_csv(f"{save_path}/shap_{gender}_{target_outcome}{count}.csv", index= False)

#%%
FILE = Path("../data/clean/untouched_dataset.csv")
d0 = pd.read_csv(FILE)
d0 = d0.drop(columns= "SA1Sex", axis=1)
...
#%% load data
def ml_featureimportance(gender, train_IDs, test_IDs):
    for count in range(10):
        ...
        if gender == "female":
            train_df = train_df[train_df['SA1Sex'] == 0]
            train_df = train_df.drop(columns= ['SA1Sex'], axis= 1)
            test_df = test_df[test_df['SA1Sex'] == 0]
            test_df = test_df.drop(columns= ['SA1Sex'], axis= 1)
        elif gender == "male":
            train_df = train_df = train_df[train_df['SA1Sex'] == 1]
            train_df = train_df.drop(columns= ['SA1Sex'], axis= 1)
            test_df = test_df[test_df['SA1Sex'] == 1]
            test_df = test_df.drop(columns= ['SA1Sex'],axis= 1)
        ...
        # LOOP
        filename = f'../results/gender_model_results/{gender}/{gender}_model_results{count}.pkl'

        if not os.path.exists(f'../results/gender_model_results/{gender}'):
            os.mkdir(f'../results/gender_model_results/{gender}')
        ...
        for outcome_name in outcome_list:
            ...
            if best_model_type == "lgbm":
                lgbm_gain_feature_importance(gender,X_train, lgbm_model, outcome_name, count)
            
            permutation_feature_importance(gender,X_train, X_test, y_test,results_dict[outcome_name][best_model_type]['pipeline'], outcome_name, count)

            shap_feature_importance(gender,X_train, X_test, results_dict[outcome_name][best_model_type]['pipeline'], outcome_name, best_model_type,count)

# %%
gender_groups = ["female", "male"]

for gender in gender_groups:
        ml_featureimportance(gender, train_IDs, test_IDs)
#%%