...
import_and_merge_features <- function(imm_status, mental_health) {
    methods <- c("shap", "permutation", "lgbm")
    dataframes <- list()
    
    # Loop through each method and import data
    for (method in methods) {
        file_path <- glue("../results/imm_feature_imp/{imm_status}/average_{imm_status}_{method}_{mental_health}.csv")
...
grf_top_features <- function(imm_status, mental_health){
    
    imm_features <- import_and_merge_features("imm", mental_health)
    nonimm_features <- import_and_merge_features("nonimm", mental_health)
    
    top_features <- union(imm_features, nonimm_features)
    
    d0 <- fread("../data/clean/imputed_processed/complete0.csv")
    d0[, imm_status := ifelse(SF3MomCan == 1 & SF4DadCan == 1, 1, 0)]
    selected_cols <- c("imm_status", "cluster", mental_health, top_features)
    d1 <- d0[, ..selected_cols, with = FALSE]
    d1 <- na.omit(d1, cols = mental_health)
    outcome <- d1[[mental_health]]
    subgroup <- d1[["imm_status"]]
    cluster <- d1[["cluster"]]
    ...
    directory_path <- "../results/subgroup_results/imm_status"
    ...
    for (colname in colnames(X)) {
       imm_features <- sapply(imm_features, function(x) ifelse(x == colname, mapping_vector[colname], x))
       nonimm_features <- sapply(nonimm_features, function(x) ifelse(x == colname, mapping_vector[colname], x))
    ...
    model_results <- list(
        ...
        nonimm_features = nonimm_features,
    ...
    saveRDS(model_results, glue("{directory_path}/imm_status_{mental_health}_model_results.rds"))

    
    
    
    direct_path <- "../figures/PDP/imm_status"
    ...
    for (cov2plot in top_features) {
        ...
        file_name <- glue("{direct_path}/imm_status_{mental_health}_{cov2plot_name}_PDP.jpg")
